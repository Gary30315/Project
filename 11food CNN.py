...
# However, not every augmentatㄉion is useful.
...
# resnet.maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
...
def get_pseudo_labels(dataset, model,threshold = 0.9):
    ...
    for batch in tqdm(data_loader):
        ...
        probs_max = torch.max(probs,1)[0]
        # ---------- TODO ----------
        # Filter the data and construct a new dataset.
        pseudo_labels = torch.max(probs,1)[1].item()
        batch[1] = pseudo_labels
